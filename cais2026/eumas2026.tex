...
\begin{enumerate}
  ...
  \item \textbf{LLM validation} (\S\ref{sec:exp5}): 3,200
    GPT-4o-mini API calls show a coupling-dependent sign flip---one
    instruction change reverses the diversity ordering ($\eta^2 = 0.291$).
...
\caption{Bridge experiment. \emph{Left:} diversity trajectories for two
iso-spectral families. Within each family (constant $\lambda_2$, varying
$\beta_1$), higher $\beta_1$ reduces early diversity (transient). Across
families (different $\lambda_2$, same $\beta_1$), higher $\lambda_2$
converges faster (persistent). \emph{Right:} temporal $\eta^2$. $\beta_1$
peaks at generation~100 and fades; $\lambda_2$ emerges at generation~200
and persists. Two invariants, two timescales.}
\label{fig:bridge-diversity}
\label{fig:bridge-eta2}
\end{figure}
...
(within-topology reversal), but absolute diversity ordering is
...
$N = 20$; scaling is open. Bridge families are approximately
iso-spectral; exact within-family matching is left to future work.
