...
across 25K~tasks and 8~protocols. Moran~\cite{moran2025error} reports
$17\times$ error amplification in unstructured communication.
...
\emph{topology} and its effect on aggregate diversity. The mapping
breaks at the mechanism level: GA migration is blind copying of
genotypes, while agent communication involves semantic understanding
and selective incorporation. This difference is precisely what makes
the sign-flip finding (Section~\ref{sec:exp5}) interesting: the same
topology can have opposite diversity effects depending on coupling.

\paragraph{Why GAs as a model system?}
GAs provide cheap, reproducible experimental control. A topology
comparison requires ${\sim}20$ seeds; at 500~generations per run, this
takes seconds. The equivalent LLM experiment requires thousands of API
calls. GAs let us sweep parameter spaces before investing in expensive
LLM validation (Experiment~5).
...
\noindent
\textit{Agent interpretation:} An arc $(u, v)$ means agent~$v$
receives agent~$u$'s output in its context window.
...
creates a pathway where agents respond to downstream effects of their
own outputs. Capucci and Myers~\cite{capucci2026feedback} formalize
the cost: each cycle requires an independent convergence guarantee.
...
\noindent
\textit{Agent interpretation:} $\lambda_2$ governs information
propagation rate. High $\lambda_2$ means fast consensus; low
$\lambda_2$ preserves local diversity longer.
...
\noindent
\textit{Agent interpretation:} $\kappa$ counts ways information can
circulate back to its source. Two digraphs with the same arc count
can have $\kappa$ from 0 (DAG) to 47, meaning different feedback at
identical bandwidth.
...
a medium-to-large effect ($\eta^2 = 0.17$) on diversity. More
feedback pathways mean faster mixing, accelerating convergence.
...
\paragraph{Interpretation.}
Simple tasks have one optimum, so topology barely matters. Complex
tasks have multiple competing strategies, and topology determines
whether the system explores many or collapses to one. The $14\times$
amplification suggests that topology effects in production agent
systems are far larger than simple benchmarks indicate. This
landscape-dependent amplification accords with Artis et
al.~\cite{artis2026heterogeneity}: environments that permit
heterogeneity reward it, and topology is the mechanism that either
...
matter? A 5-node ring and star both have $\beta_1 = 1$ and $|E| = 5$,
...
\paragraph{Interpretation.}
The star's higher $\lambda_2$ ($1.0$ vs $0.382$) enables faster
consensus, but the effect is modest ($\eta^2 \leq 0.13$, none
significant after generation~100) compared to cross-$\beta_1$ effects
($\eta^2 = 0.17$--$0.69$). This confirms that \textbf{cycle rank is
the primary invariant}, with degree distribution secondary. The ring
advantage plateaus at $\eta^2 \approx 0.08$--$0.13$ for $K \geq 2$,
contrasting with the monotonic cross-$\beta_1$ dose-response.
...
\paragraph{Interpretation.}
$\beta_1$ and $\lambda_2$ control diversity through independent
mechanisms: $\beta_1$ determines initial mixing speed (transient,
first ${\sim}100$~generations), while $\lambda_2$ governs long-term
consensus rate (persistent, generation~200+). For agent systems:
\emph{cycle rank controls the exploration window}, while
...
\textbf{random initialization} (high entropy). GA migration is
colimit-like (merging $\to$ homogenization); LLM ``be different''
context is limit-like (accumulating constraints $\to$ agents pushed to
distributional tails). The topology is invariant; the sign depends on
coupling direction. The formal categorical construction is developed
...
Of these nine systems, six are restricted to DAGs ($\beta_1 = 0$).
Only HERA~\cite{hera2026} tracks cycle count, but without density
control. The assumption that acyclic structures are preferable has
never been tested --- and Ao et al.~\cite{ao2026reliability} show the
assumption has a formal cost: DAG-constrained multi-agent systems are
decision-theoretically dominated by centralized planning (Proposition~6
of their paper), motivating the investigation of what cycles recover.
...
The sign-flip has direct implications: (1)~merging coupling
homogenizes (more connectivity $\to$ less diversity); constraining
coupling diversifies (more connectivity $\to$ more diversity).
(2)~Independent LLM agents produce near-identical outputs; ``ensemble
diversity'' without topology intervention is illusory. (3)~Moran's
$17{\times}$ error amplification~\cite{moran2025error} arises from
high $\beta_1$ with merging coupling; the standard DAG fix
simultaneously removes cycles \emph{and} density.
...
Our LLM experiment does \emph{not} control density: the sign-flip is
robust (within-topology reversal), but absolute diversity ordering is
confounded. Density-controlled LLM experiments are needed. The
diversity metric (cosine distance on embeddings) lacks human
validation. GA experiments use $N = 20$; scaling is open. Bridge
families are approximately iso-spectral ($\lambda_2 = 0.586$ vs
$1.0$); exact matching is left to future work.
...
without density control. Our Experiment~1 is, to our knowledge, the
first density-controlled comparison.
Brewster et al.~\cite{brewster2025diversity} provide a theoretical
foundation for why topology matters: in evolutionary graph theory,
absorption time (time until a single type dominates)
scales as $\Theta(N^2)$ on complete graphs, $\Theta(N^3)$ on cycles,
and superexponentially on certain directed structures.
The cycle--vs--complete gap directly predicts our diversity ordering:
slower absorption preserves heterogeneity longer.
...
Maximum Heterogeneity: any distributed production system optimising for
performance converges on an increasingly heterogeneous configuration,
with communication topology determining heterogeneity's spatial
distribution. Applied across biology, economics, and computing, this
cross-disciplinary evidence strengthens our finding that ring topologies
(which preserve local diversity) outperform fully connected graphs on
rugged landscapes.
...
Langer and Turing~\cite{langer2026games} develop the categorical
