...
% Draft v8: April 10, 2026
% V8: Address peer review. Fix "one word" → "one instruction". Downgrade Kan
%   extension claims to analogy + companion paper ref. Add metric validation.
%   Add limitations paragraph. Acknowledge LLM density confound. Tighten Exp 2.
%   Remove unsourced industry stats. Discuss expression evaluator boundary case.
...
8~coding tasks show that topology modulates solution diversity in
GPT-4o-mini agents, with the effect sign determined by the coupling
instruction: negative coupling (``use a different algorithm'') inverts
the GA diversity ordering (Cohen's $d$ from $-14.0$ to $-1.9$ across
six tasks); positive coupling (``build on and improve'') restores it
($\eta^2 = 0.291$, $p < 10^{-19}$). A single instruction change flips
which topology produces the most diverse output. The sign depends on
whether coupling merges neighbors' solutions (homogenizing, as in GA
migration) or excludes them (diversifying, as in the ``be different''
instruction). At least nine deployed systems chose topologies without
...
and LLM agents; (2)~the effect \emph{inverts} between GAs and agents,
explained by whether the coupling mechanism merges or constrains;
(3)~at least nine deployed systems chose topologies without controlled
comparisons.
...
orchestration is direct at the graph level:
...
directed channels. The analogy is weaker at the node level---a GA
island contains a \emph{population} evolving over many generations,
while an agent produces a single output---but the diversity--topology
...
On NK landscapes with tunable ruggedness (250 runs: $K \in \{0, 2, 4,
6\}$, topologies selected to span the $\kappa$ range from
Experiment~1, 10--20 seeds per condition), topology's effect scales
with difficulty:
...
The ``none'' topology provides an implicit sanity check: all five
agents generate independently with no information sharing, yet
produce near-identical outputs (mean diversity 0.090 across tasks,
$\sigma \approx 0.01$)---confirming the metric detects the known
homogeneity of the model's prior distribution. Under connected
topologies with negative coupling, where agents are explicitly
instructed to use different algorithms, diversity rises substantially.
The low within-condition variance also explains the large Cohen's $d$
values reported below: the effect is clean, not an artifact of the
scale.
...
$p < 10^{-6}$; Cohen's $d$ ranges from $-14.0$ to $-1.9$). Two
exceptions are informative: Bloom filter ($d \approx 0$, a unimodal
task with essentially one canonical implementation---no algorithmic
variety for topology to act on) and expression evaluator ($d = +0.4$,
positive: the model's prior is already spread across multiple parsing
strategies). The expression evaluator confirms the mechanism's boundary
condition: when the prior is already flat (high entropy), isolation
produces diverse solutions---the GA regime---and negative coupling adds
little. The sign flip requires a peaked prior.
...
(homogeneous). The inversion follows from the coupling direction:
...
direction. We conjecture that this sign reversal is an instance of the
adjunction between left and right Kan extensions along the topology
functor---colimit (merge) vs.\ limit (constrain)---with the formal
categorical construction developed in a companion
paper~\citep{langer2026games}.
...
of ``use a different algorithm.'' 1,600 additional API calls (total:
3,200). Results: the diversity ordering \textbf{reverts to GA-like}:
none $>$ star $\approx$ ring $>$ FC (Kruskal-Wallis $H = 94.83$, $p =
2.01 \times 10^{-20}$; Cohen's $d = 1.08$, none vs.\ FC; $\eta^2 =
0.291$). Crucially, the \emph{none} topology---where agents have no
neighbors---shows identical diversity across conditions (negative:
0.090, positive: 0.095, $p = 0.96$), confirming that coupling
direction, not some other experimental variable, drives the reversal.
Connected topologies collapse to near-zero diversity (0.025--0.028)
under positive coupling, as agents converge on a shared best solution.
A single instruction change flips the diversity ordering---precisely as
the colimit/limit framework predicts.
...
\emph{(1)~The sign matters.} A single instruction change flips which
topology produces the most diverse output ($\eta^2 = 0.291$, $p <
10^{-19}$). Predictions based on GA intuition alone will have the
wrong sign for diversifying agents.
\emph{(2)~Premature consensus.} Isolated LLM agents converge to
identical solutions (Cohen's $d$ from $-14.0$ to $-1.9$ vs.\ connected
agents across six tasks). ``Ensemble diversity'' claims based on
independent agents may be illusory when models share a peaked prior.
...
\begin{enumerate}
  ...
  \item \textbf{Know your coupling direction.} A single instruction
    change flips the diversity ordering. Merging neighbors' outputs
    homogenizes; differentiating from them diversifies.
...
Our GA experiment (Experiment~1) controls density by using iso-dense
directed graphs. Our LLM experiment does not: the four topologies
(none, ring, star, fully connected) differ in both density and
structure. The \emph{sign-flip} finding is robust to this confound,
since both coupling conditions use identical topologies and the
reversal is within-topology. However, the absolute diversity ordering
across topologies in the LLM experiment is confounded by density---the
same critique we level at prior work. Density-controlled LLM
experiments (using iso-dense directed topologies) are needed to fully
isolate the mechanism. Additionally, our diversity metric (cosine
distance on sentence embeddings) is validated only implicitly via the
``none'' baseline; a systematic comparison against human-judged
algorithmic similarity would strengthen the measurement.
...
Langer and Turing~\citep{langer2026games} develop the categorical
framework (lax monoidal functors, Kleisli morphisms over an evolution
monad) that motivates the colimit/limit interpretation of the sign
flip; our Experiment~3 provides the first empirical test of their
predicted coupling-sign dependence.
...
on hard ones. In LLM agents, the effect is equally strong but its
\emph{sign} depends on coupling direction: negative coupling (``be
different'') inverts the GA ordering ($d$ from $-14.0$ to $-1.9$);
positive coupling (``build on'') restores it ($\eta^2 = 0.291$). The
sign is determined by whether coupling merges solutions (colimit-like,
homogenizing) or constrains them (limit-like,
diversifying)---confirmed by the null control: isolated agents show
identical diversity regardless of coupling instruction ($p = 0.96$).
At least nine deployed systems chose topologies based on comparisons
that simultaneously varied the property they claimed to test. For safe
multi-agent deployment, topology must be tuned deliberately, its
coupling direction understood, and its effects monitored through
diversity metrics---not assumed away.
...
\bibitem[Langer and Turing(2026)]{langer2026games}
R.~Langer and C.~Turing.
\newblock From games to graphs: Categorical composition of genetic algorithms
  across domains.
\newblock In \emph{Proc.\ ACT}, 2026.
