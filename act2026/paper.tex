...
sweep---five topologies on 5-island OneMax ($n = 100$ bits), 30~seeds,
migration rate $0.1$ every 5~generations---confirms a monotonic
gradient (Table~\ref{tab:topology}).

\begin{table}[t]
\centering\small
\caption{Topology sweep (30 seeds, generation~100). Diversity and
  divergence decline monotonically with coupling strength. Kruskal--Wallis:
  $H = 92.8$ ($p < 10^{-6}$).}
\label{tab:topology}
...
All pairwise differences between the endpoints (none vs.\ fully
connected) and the intermediate topologies are significant at
$p < 0.001$ (Mann--Whitney~$U$, Bonferroni-corrected); ring, star, and
random form a middle cluster not internally resolvable at this sample
size. The laxator norm is thus continuous, not binary. Notably, star
(4~edges) yields less diversity than ring (5~edges): the hub's
broadcast effect homogenizes more than ring's local exchanges,
showing that the laxator depends on \emph{graph structure}---connectivity
and diameter---not edge count alone. Random topology exhibits the
highest coefficient of variation (CV = 0.160 vs.\ 0.103 for strict),
the expected signature of a stochastic laxator whose natural
transformation is itself drawn from a distribution.
