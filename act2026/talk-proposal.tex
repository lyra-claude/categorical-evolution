...
fingerprint across two unrelated domains ($d = 4.34$, $p < 3.69 \times
10^{-11}$). We conjecture that strict composition preserves such
...
We tested four composition strategies on two unrelated domains: evolving
game-playing evaluation functions for checkers and evolving maze
topologies. Each strategy applies the same operators in a different
compositional arrangement. The central finding is that each composition
produces a characteristic \emph{diversity fingerprint}---a temporal
pattern in population diversity invariant across domains. We observe four
distinct fingerprints: monotonic decline, spike-crash-rebound, stable
oscillation, and spike-collapse.

Statistical validation over 30 seeds confirms cross-domain stability:
Cohen's $d = 4.34$ ($p < 3.69 \times 10^{-11}$). Compositional
structure is the dominant predictor of diversity dynamics, not
