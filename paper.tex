...
\subsection{Categorical Approaches to Evolutionary Computation}

The intersection of category theory and evolutionary computation is nearly empty. Jia et al.~\cite{jia2025} survey categorical approaches to machine learning across four perspectives (gradient-based learning, probability, invariance, compositionality), yet evolutionary computation is not covered --- an explicit gap in the literature that this work begins to fill.

The closest related work is Warrell et al.~\cite{warrell2024}, who use the Kantorovich monad to model multilevel population structure in genetic algorithms. Their framework captures population hierarchy via Wasserstein distances: populations are probability distributions over genotypes, and hierarchical populations (demes, species) are distributions over distributions. This is complementary to our approach. Warrell et al.\ model \emph{what populations are} (hierarchical probability distributions); we model \emph{what operators do} (Kleisli morphisms that compose). Their framework does not represent GA operators as categorical morphisms and has no analog of our diversity fingerprints or the strict/lax dichotomy. Conversely, our framework does not capture the measure-theoretic structure of population distributions that the Kantorovich monad provides. A synthesis --- Kleisli composition of operators acting on Kantorovich-structured populations --- is a natural direction for future work.
...
\textbf{Kleisli--Kantorovich synthesis.} Our Kleisli composition of operators and Warrell et al.'s~\cite{warrell2024} Kantorovich monad for hierarchical populations address orthogonal aspects of evolutionary computation. Composing Kleisli arrows over Kantorovich-structured population spaces would unify operator composition with population-level measure theory --- a richer categorical framework for multilevel evolutionary systems.
...
\bibitem{warrell2024}
Warrell, J., Kuelheim, C., \& Moran, G.\,F. (2024).
A categorical framework for multilevel evolution of genetic algorithms using the Kantorovich monad.
\textit{arXiv:2411.09779}.

\bibitem{jia2025}
Jia, P., Liang, T., \& Chen, C. (2025).
Category theory and machine learning: A survey.
\textit{Axioms}, 14(2), 146.
arXiv:2408.14014.
