...
This formalization is not merely notational. It yields four concrete results:
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Cycle rank as topological predictor.} The first Betti number $\beta_1$ of the migration graph outperforms algebraic connectivity $\lambda_2$ as a predictor of fitness dynamics ($\rho = 0.893$, $p = 0.007$), resolving anomalies where $\lambda_2$ mispredicts (e.g., the star topology).
\end{enumerate}
...
Every GA operator draws random numbers, reads configuration (tournament size, mutation rate), and logs statistics. We bundle these effects into a monad stack $M = \texttt{ReaderT GAConfig (WriterT GALog (State StdGen))}$. A \emph{Kleisli arrow} $a \to M\,b$ takes input~$a$, produces output~$b$ wrapped in~$M$. Kleisli composition ($\mathbin{>\!\!=\!\!>}$) chains arrows while threading effects automatically. The monad laws guarantee associativity: re-parenthesizing a pipeline does not change behavior~\cite{maclane1971categories, moggi1991notions}.
...
\subsection{Cycle Rank as Topological Predictor}
\label{sec:cycle-rank}

While $\lambda_2$ (algebraic connectivity) predicts the mean-fitness ordering across domains ($\rho = 1.0$ in the topology sweep), it is a global spectral summary that conflates tree-like and cyclic connectivity. A finer predictor emerges from graph homology. The \emph{cycle rank} (first Betti number) of a migration graph $G = (V, E)$ with $c$ connected components is $\beta_1(G) = |E| - |V| + c$---the number of independent cycles. Each cycle provides an edge-disjoint circulation pathway for genetic material; trees ($\beta_1 = 0$) have exactly one path between any pair of vertices, creating bottlenecks. On an 8-island OneMax configuration (7 connected topologies, 10~runs each), $\beta_1$ is the only predictor significant at $p < 0.01$: Spearman $\rho = 0.893$ ($p = 0.007$) vs.\ $\lambda_2$'s $\rho = 0.679$ ($p = 0.094$). Critically, $\beta_1$ resolves two anomalies: the \emph{star} ($\lambda_2 = 1.0$ but $\beta_1 = 0$, ranking 7th---three below $\lambda_2$'s prediction) and the \emph{barbell} ($\lambda_2 = 0.07$ but $\beta_1 = 6$, ranking 3rd---four above). In the island-functor framework, each independent cycle provides an alternative composition path for the laxator: cycle-completing edges contribute parallel pathways that reduce variance in information flow. Two independent results corroborate: Chen et al.~\cite{chen2026cascade} report a cycle-rank norm raising accuracy from 50.4\% to 87.2\% in multi-agent routing, and Patel et al.~\cite{patel2026adaptorch} show topology importance grows as individual agents improve.
...
We adopt \emph{strict/lax} by analogy with monoidal functors: the island functor preserves composition exactly when islands are isolated (strict), and only up to a migration morphism when coupled (lax).
...
\paragraph{Cycle rank outperforms $\lambda_2$.}
A finer-grained analysis on 8-island OneMax (Section~\ref{sec:cycle-rank}) reveals that cycle rank $\beta_1$ outperforms $\lambda_2$ as a single-variable predictor of fitness dynamics ($\rho = 0.893$, $p = 0.007$), resolving the star anomaly ($\lambda_2 = 1.0$ but $\beta_1 = 0$) and the barbell anomaly ($\lambda_2 = 0.07$ but $\beta_1 = 6$). Figure~\ref{fig:two-panel} shows the relationship between $\beta_1$ and mean fitness alongside the decay of topology's explanatory power ($\eta^2$) over generations. Independent multi-agent results corroborate: Graph-GRPO's DAG topologies ($\beta_1 = 0$) achieve 92.45\% on code generation benchmarks~\cite{cang2026graphgrpo}, while BIGMAS's cyclic topologies ($\beta_1 > 0$) reach 100\% on Game of 24~\cite{hao2026bigmas}. BIGMAS's failure analysis is instructive---failed runs exhibit excess orchestrator decisions (9.4 vs.\ 7.3 per task), suggesting that too many cycles degrade performance through redundant coordination, consistent with a Goldilocks zone for $\beta_1$.

\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth,height=0.35\textheight,keepaspectratio]{gecco_two_panel.pdf}
\caption{Left: Cycle rank ($\beta_1$) vs.\ mean fitness at generation~30 across 8 topologies ($\rho = 0.893$, $p = 0.007$). Right: Topology effect ($\eta^2$) over generations---88\% at gen~30, ${<}10\%$ by gen~50.}
\label{fig:two-panel}
\end{figure*}
...
The laxity magnitude $D^*$ in item~2 is a property of the migration topology, not a universal constant. For the ring and star topologies tested here, $D^* \approx 0.78$; other topologies will yield different values determined by the spectral gap $\lambda_2$ of their respective Markov chains. The claim is that $D^*$ is invariant to \emph{boundary position and migration frequency} within a fixed topology---not that it is the same across topologies. The cycle-rank analysis (Section~\ref{sec:cycle-rank}) suggests $\beta_1$ may be a finer predictor of $D^*$ than $\lambda_2$: topologies with the same $\lambda_2$ but different $\beta_1$ (e.g., star vs.\ barbell) exhibit different convergence dynamics, and each independent cycle provides an additional composition path through which the laxator can redistribute genetic material. A full characterization of $D^*$ as a function of both $\lambda_2$ and $\beta_1$ is deferred to future work.
\end{remark}
...
Zhang et al.~\cite{zhang2025monadic} independently formalize monad stacks for LLM agent context engineering but do not address topology or cycle rank.
...
Composition determines behavior. We have demonstrated this across eight domains with four results: diversity fingerprints are stable signatures of composition pattern, not of individual operators or genome types---with topology explaining $23.9\times$ more variance than domain across five independent problems; the island functor exhibits a sharp strict/lax dichotomy at zero migration---confirmed across 30~independent seeds with Cohen's $d > 4.0$ ($p < 10^{-10}$)---with practical consequences for checkpointing and distributed execution; cycle rank ($\beta_1$) outperforms algebraic connectivity ($\lambda_2$) as a topological predictor of fitness dynamics ($\rho = 0.893$, $p = 0.007$), with independent corroboration from multi-agent routing~\cite{chen2026cascade} and orchestration scaling~\cite{patel2026adaptorch}; and three levels of composition---operators, pipelines, strategies---share the same Kleisli algebraic structure, giving a recursive design algebra for genetic algorithms.
