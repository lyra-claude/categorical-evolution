...
A topology sweep across four domains---OneMax (binary), maze navigation (binary), graph coloring (integer), and knapsack (binary)---reveals that composition structure determines diversity dynamics with striking universality. Island-model GAs were run on five migration topologies (none, ring, star, random, fully connected) with 30 independent seeds per condition. All four domains produce the identical mean-fitness ordering:
\begin{center}
\small
none $<$ ring $<$ star $<$ random $<$ fully\_connected
\end{center}
\noindent with pairwise Spearman rank correlation $\rho = 1.0$ across all $\binom{4}{2} = 6$ domain pairs. A two-way ANOVA finds that topology explains $23.9\times$ more variance in mean fitness than domain ($F_{\mathrm{topo}} = 47.8$, $p < 10^{-6}$; $F_{\mathrm{domain}} = 0.13$, $p = 0.945$). The domain factor is non-significant: it contributes essentially no explanatory power beyond topology. This ordering follows the algebraic connectivity $\lambda_2$ of the migration graph~\cite{sanz2026algebraic}, confirming a structural prediction from coupled oscillator theory. A fifth domain (sorting networks) violates the ordering, providing a falsifiable scope condition: the invariant holds for domains where the fitness landscape admits sufficient selective gradient, and breaks where domain-specific constraints dominate migration dynamics.

The key result is that \emph{composition structure is the invariant}. Across four domains spanning binary, integer, and real-valued genomes, the same topological composition produces the same diversity and fitness ordering. The fingerprint follows the functor, not the object.
...
The topology sweep (Section~4.2) extends domain coverage to six problems (OneMax, maze, graph coloring, knapsack, checkers, sorting networks), with four showing identical topology ordering. Extending to permutation-based domains (TSP, scheduling), variable-length genomes (NEAT-style neuroevolution~\cite{eiben2015introduction}), and high-dimensional continuous optimization would further test universality. The sorting network exception suggests that domain-specific constraints can override compositional effects---characterizing this boundary precisely remains future work.
...
Composition determines behavior. We have demonstrated this across six domains with three results: diversity fingerprints are stable signatures of composition pattern, not of individual operators or genome types---with topology explaining $23.9\times$ more variance than domain across four independent problems; the island functor exhibits a sharp strict/lax dichotomy at zero migration---confirmed across 30~independent seeds with Cohen's $d > 4.0$ ($p < 10^{-10}$)---with practical consequences for checkpointing and distributed execution; and three levels of composition---operators, pipelines, strategies---share the same Kleisli algebraic structure, giving a recursive design algebra for genetic algorithms.
