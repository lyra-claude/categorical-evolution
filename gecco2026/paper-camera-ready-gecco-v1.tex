...
\acmConference[GECCO '26]{The Genetic and Evolutionary Computation Conference 2026}{July 13--17, 2026}{San Jos\'{e}, Costa Rica}

% amsmath, amssymb, amsthm, booktabs are loaded by acmart
\usepackage{amssymb}  % explicit load needed for \ggg
\usepackage{tikz-cd}
...
This formalization is not merely notational. The framework---a three-level composition tower where operators compose into pipelines, pipelines into strategies, and strategies into multi-population systems, all via the same Kleisli mechanism---yields two empirical results:
...
\Description{Bar chart showing mean diversity at generation 99 across six domains (OneMax, Maze, Graph Coloring, Knapsack, No Thanks, Checkers) for five migration topologies. The ordering none greater than ring greater than star greater than random greater than fully connected is identical in every domain.}
...
\]
with perfect concordance: Kendall's $W = 1.0$, $\chi^2 = 24.0$, $p = 0.00008$; all 15 pairwise Spearman $\rho = 1.0$ (Figure~\ref{fig:multi-domain-topology}). A two-way ANOVA confirms that topology explains $23.9\times$ more variance in diversity dynamics than domain ($F_{\mathrm{topo}} = 47.8$, $p < 10^{-6}$; $F_{\mathrm{domain}} = 0.13$, $p = 0.945$).
\end{observation}
...
\end{tikzpicture}
\Description{Three line plots showing diversity trajectories over 50 generations for four composition strategies (flat, hourglass, island, adaptive) across maze generation, graph coloring, and knapsack domains. Each strategy produces a distinct fingerprint pattern that is consistent across all three domains.}
...
\paragraph{Practitioner fingerprints.}
...
\paragraph{Prefer ring or mesh topologies over star topologies.} Both yield $\beta_1 \geq 1$ and avoid the composition-tax singularity that accelerates premature convergence. A star with $n$ leaves has $\beta_1 = 0$; replacing the hub with a ring costs zero additional edges while adding $n-1$ independent cycles.

\paragraph{Monitor $\lambda_2$ rather than diversity directly.} Algebraic connectivity is cheap to compute ($O(n^2)$ per generation) and predicts diversity collapse 50--100 generations before it manifests in the population. A dropping $\lambda_2$ is an early warning; a flat diversity curve is confirmation after the fact.

\paragraph{Prefer cycle-closing edges over increased migration rate.} If $\lambda_2$ drops below a threshold (empirically: 0.1 for $n \leq 20$), add one edge between the two most isolated subpopulations rather than increasing migration rate. Increased migration raises coupling without changing topology; a cycle-closing edge changes both.
...
Recent work uses large language models as evolutionary operators. AlphaEvolve~\cite{alphaevolve2025} uses LLM-driven code evolution to discover novel algorithms across mathematics and computer science, demonstrating that composition choices (which code components to evolve, how to combine LLM-generated mutations) determine the quality of discovered solutions. CodeEvolve~\cite{assumpcao2025codeevolve} uses three modular LLM operators on an island-based GA. Kim et al.~\cite{kim2025scaling} find error amplification of $17.2\times$ for independent (lax) vs.\ $4.4\times$ for centralized (strict) multi-agent compositions. Van Stein et al.~\cite{vanstein2025code} analyze code evolution graphs in LLM-driven algorithm design, finding that different LLMs produce structurally distinct search trajectories---suggesting that the ``operator'' (which LLM generates mutations) shapes the evolutionary path.

\paragraph{Topology optimization in multi-agent systems.}
Yang et al.~\cite{yang2025topology} argue that topological structure learning is an underresearched priority for LLM-based multi-agent systems, documenting up to 10\% performance variation across fixed topologies. Our results strengthen this claim: migration topology explains $23.9\times$ more variance than domain in evolutionary diversity dynamics, and the topology ordering is universal across six domains.
